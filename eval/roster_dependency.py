...
**注意（§44）**: 「人数なし」とは席上限を外すことである。席の音声で決め直す
経路（ラベル不純・ラベル継続）は人数の設定に依存せず、人数なしでも動く。
当初この経路まで止めた条件を「人数を使わない」と呼んで 67.5% と報告したが、
それは本番の挙動ではなかった（§44 で訂正。正しくは 85.5%）。3行目は
その参考条件として残す。
...
CONDITIONS = [
    ("決めている（本番）", True, True),
    ("人数なし（本番のまま）", False, True),
    ("参考: 席の割当ても止める", False, False),
]
