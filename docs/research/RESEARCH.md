...
### RQ と H の対応表 (2026-07-07 追記)

> この節の作成: Claude Fable 5 (claude-fable-5), 2026-07-07

`significance_2026-07.md` で立てた理論駆動の仮説 H1〜H4 と、上記 RQ1〜4 は別の粒度・別の軸で
併存している (`research_review_2026-07-07.md` §1 が指摘する「階層のねじれ」)。
`research_masterplan_2026-07-07.md` §0 の決定に従い、**修論の序論では H 群を主軸に据え、
RQ 群はその操作化として従属させる**。以下は対応関係・検証手段・現時点 (2026-07-07) の状態を
1 枚にまとめたもの。

| H (理論仮説) | 内容 | 操作化 (RQ) | 検証手段 | 現在の状態 (2026-07-07 時点) |
|---|---|---|---|---|
| **H1** | 未共有情報の表出促進: 関係ラベル付き evidence 提示は、ラベルなし提示 (flat_rag) より未共有情報の言及率と stance-aligned citation を高める | RQ4 の一部 (citation_rate〔source 別〕、cross_source_edge_rate、stance-aligned citation) | シミュ本実験 (dose 統制 ablation: full_proposal vs full_proposal_unlabeled) ＋ 対面実験 (案1: hidden profile 型 3 条件ラテン方格) | シミュ: 指標実装済み・盲検化済み、7〜8月に本実験実施予定。対面: 教材設計中 (9月確定予定、`stage_c_materials_design_2026-07.md`)。実験本体は 10〜11月で未実施 |
| **H2** | 未応答反論の回収: 「未応答の反論」の提示は `pct_attacks_answered` を高める (脱人格化仮説の操作的検証) | RQ1 の一部 (`pct_attacks_answered`) | シミュ本実験 (同上) ＋ 対面実験 (案1) | 同上。指標は観測用 AF から決定的計算可能 (E4 実装済み、LLM judge 不要のため信頼性が高い) |
| **H3** | 過剰自信の緩和・対面固有性 (WM 外在化): 支持/攻撃の構造的バランス提示で反対意見理解と過剰自信が変化する。対面固有には、AF 提示が集団の記憶を代替し重複主張率が下がり忘却反論の復活率が上がる | RQ2 (`confidence_change`, `opposition_understanding`) ＋ 対面固有の構造指標 (重複主張率・忘却反論復活率、`significance_2026-07.md` §5) | シミュでは機構確認のみ (LLM ペルソナの自己申告値、妥当性に留保あり)。対面実験で人間の主観指標＋構造指標を測定 (限界を考察で明記) | シミュ: judge 指標として稼働中だが LLM ペルソナ・LLM judge 双方の妥当性は未検証 (`research_review_2026-07-07.md` §4(b))。対面: 未実施、構造指標2種は設計段階 |
| **H4** | 手続き介入の受容性: 手続き介入 (invite・脱線復帰) は内容介入 (fact 補正) より受容される | RQ3 の一部 (介入の透明性・受容度) | 対面実験の介入ログ事後分類 (案3: 案1のログをそのまま流用、追加コストほぼゼロ) | 未着手。案1完了後 (11月予定) に着手予定。手続き/内容の ablation 軸は実装内に既存 (`logic_review_2026-07.md` A章) |

**注記(対応が 1 対 1 でない点)**: RQ1 (議論の質全体) と RQ3 (介入の透明性) は、上表の通り H2・H4 に
部分的に従属する一方で、シミュレーションの judge 指標 (`intervention_transparency` など) としては
H 群とは独立に測定され続ける。この非対称性は意図的なもので、序論では「H 群＝修論が最終的に
主張する理論的貢献」「RQ 群＝実装から自然に出てくる評価軸であり、H 群を測るための操作化として
再利用されるものと、シミュレーション固有の補助指標として残るものが混在する」と明示的に書き分ける。
