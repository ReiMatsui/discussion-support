...
## 4.2 実マイクでの一人試行（2026-09-13、3回、`--live-listen always`）

内蔵スピーカーで再生し、回り込みがある条件。松井が一人で話し、呼びかけと
被せを試した。

- 18:34（旧版）: 全二重の割り込みは効く（被せた瞬間に「承知。う」で切れる）が、
  こちらの STT 側割り込み検出と「中断内容の再送」がモデル側の続きと二重になり、
  発話が3本に割れ、文字も混ざった。01:28 に指示なしで口を挟んだ例が1回。
  → `listen="always"` ではこちらの割り込み処理と再送を止め、モデルに任せる。
- 18:46: 無断の発話なし（5回とも直接の呼びかけへの応答）。「ちょっと、ちょっと」
  に対しては文を言い切ってから「はい、黙ります」。記録の混ざりは消えた。
- 18:51（終端 1.2 秒版）: 「一回待ってください」で文の途中で「了解」と止まり、
  「もう一回しゃべってください」で「はい、続けます」と再開。発話は1本ずつ。
  Soniox は AI の声を拾うが声紋フィルタで議事録には入らない（9回除去）。
  GPT-Live が自分の声に反応した形跡はない。

結論: 内蔵スピーカーの回り込みがあっても、割り込みの検出・停止・再開は
モデル側で成立する。`always` では STT 側の割り込み検出・譲歩・再送は使われず、
残る自前の処理は議事録用の声紋フィルタだけになる。

「話者分離が停止しました」の警告は3回とも誤報（分離の区間は最後まで届いて
いた）。UI の「会議を開始」で分離の接続を作る処理と送信スレッドの競合で、
初回接続が済む前は警告しないように修正（`_audio_io.py`）。

## 5. 判断

- 割り込みの扱いは `--agent-engine live --live-listen always` が Realtime 版より
  単純で自然（§4.2）。実験で使うかは、2〜3人の試行で「止まらなかった回数」と
  「呼びかけ以外で口を挟んだ回数」を数えてから決める。統制の観点では後者が
  ゼロに近いことが条件。切り替えるなら実験開始前に決め、途中では変えない。
- Realtime 版は既定のまま残す（比較対象と、Live が不安定なときの戻り先）。
