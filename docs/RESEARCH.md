...
議論ログと外部知識を **単一の議論グラフ (Argumentation Framework, AF) として統合的に保持・運用** し、論証構造の連続性を担保するマルチエージェント議論支援システムを構築する。これにより、確証バイアスや議論の極化を構造的に抑制し、合意形成の質を高めることを目指す。

### リサーチクエスチョン

- **RQ1 議論の質への影響**: 統合議論グラフを活用する議論支援は、フラット RAG や支援なしと比べて合意形成の質 (満足度・根拠の豊富さ・情報活用度) にどう影響するか
- **RQ2 反対意見の理解と確証バイアスの緩和**: 賛成・反対の論証を構造的にバランスして提示することで、反対意見の理解度や過剰な自信は変わるか
- **RQ3 介入の透明性**: 介入根拠が論証構造として明示されることで、納得感や情報活用度にどう影響するか
...
| C4 | 事前資料の網羅性の限界 | Web 検索エージェント (M3 で実装予定) |
...
- ⏳ Stage 2 (DQI-inspired 主観評価): justification_level / respect_score / constructiveness など
- ⏳ M3: Web 検索エージェント
