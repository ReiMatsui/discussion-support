...
# 0.65→0.70（2026-07-17, ユーザー判断）: 失敗の非対称性による。閾値過大の損害は
# 「確定の遅れ」（蓄積継続で回復可能）、過小の損害は「恒久汚染」（確定は不可逆）
# のため、誤確定側(≤0.62)との余白を広く取る。正しい確定6件(≥0.72)は全て通る。
PYANNOTE_CLUSTER_CONFIRM_MIN_SIM = 0.70
