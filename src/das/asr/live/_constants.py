...
直近の会話に、今すぐ短く補足すべき明確な事実誤りがあるか判定してください。
...
介入してよい例:
- 定義、計算式、単位、数値、統計、日付、制度名などが高確信で誤っている
...
{{
  ...
  "claim": "誤っている主張。なければ空文字",
  "correction": "会話でそのまま言える短い補足。なければ空文字",
  "reason": "短い理由"
}}"""
...
_FACTCHECK_CHECK_SEC = 2.0    # LLM判定の最小間隔（連続呼び出しを抑制）
_FACTCHECK_COOLDOWN = 18.0    # 訂正介入の最小間隔（しつこさ防止）
