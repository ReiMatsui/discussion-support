...
# 短い発話で1位と2位が僅差のときは、名前を出さず未確定にする（handoff §36）。
# 残る誤帰属の81%は1秒未満の発話で、その大半がこの席の割当てを通る。0.5秒の
# 「ほう。」から取った埋め込みでも rank-1 は必ず1席を選ぶためである。
# 校正（Chiba 9ラン・1788発話・正解は文章の一致で当てた実測）:
#   差0.03・1秒未満だけ → 誤帰属 12.6→9.2% / 正解 83.5→81.4%（棄権の63%が誤り）
#   差0.05・1秒未満だけ → 誤帰属 12.6→8.1% / 正解 83.5→79.9%（同 56%）
#   差0.10・1秒未満だけ → 誤帰属 12.6→5.4% / 正解 83.5→75.6%（同 48%）
# しきい値を上げるほど取引は悪くなる（63→56→48%）。1秒以上に同じ差を掛けると
# 棄権の70%が正解の取りこぼしになるので、長さで切る。
# **当たるようになるのではない**——相槌の長さの発話で、誤った名前の代わりに
# 未確定を出す交換である（文字数では 誤帰属 5.5→4.5% / 正解 91.6→91.0%）。
SEAT_AUDIO_SHORT_MS = 1000
SEAT_AUDIO_SHORT_MIN_MARGIN = 0.03
# 席の割当てだけに使う声紋モデルのサイズ（ReDimNet。handoff §38）。
# 声紋層（classify）は b2 のしきい値で校正されているので触らない。席の割当ては
# **しきい値を使わない**（席を持つ人の中で1位を選ぶだけ）ため、ここだけ替えても
# 再校正が要らない。
# 校正（Chiba 9ラン・1788発話・正解は文章の一致で当てた実測。1秒未満の帯）:
#   b2 正解 66.8% / 誤帰属 16.8%   埋込 150ms
#   b3 正解 68.6% / 誤帰属 16.8%   埋込 399ms  ← b2 と変わらない
#   b5 正解 71.0% / 誤帰属 14.6%   埋込 553ms  ← ここが上限
#   b6 正解 70.3% / 誤帰属 14.6%   埋込 848ms  ← b5 と同等で1.5倍遅い
# 全体では 正解 81.5→83.1% / 誤帰属 9.2→8.4%。1秒以上は不変（91.5→91.3%）。
# 埋込msは測定機（クラウドCPU）の値。遅ければ "b2" に戻せば元の挙動に戻る。
SEAT_AUDIO_EMBED_SIZE = "b5"
# 未登録話者の門番（handoff §47）: ラベル不純の発話が「長いのに、どの登録済み
# プロファイルとも十分似ていない」なら、写像や席で誰かへ寄せず未確定に倒す。
# 未登録の声はプロファイルも席も無く、既存の誰かに吸われる以外の出口が無い
# （2026-07-30 講義: 未登録の話者6発話・915字が参加者Bへ）。
# 校正（2026-07-30 講義 + 校正済み実会話9本の diag 実測）:
#   講義: 該当7件中5件が実際の誤り（907字回収）。巻き添えは30字1件+要確認1件
#   実会話9本: ラベル不純1377件中、該当0件——登録済みの人しかいなければ、
#   長い発話の best sim は 0.5 を割らない（誤りの最大 0.486 / 正解の最低 ≈0.5）
# 短い発話に広げない理由: 短い発話は正解でも sim が低く出る（講義の正解Bに
# 0.18〜0.39 が複数）。長さの下限が巻き添えを防いでいる。
IMPURE_LOWSIM_MIN_CHARS = 30
IMPURE_LOWSIM_MAX_SIM = 0.5
# 席の回収（handoff §49）: 開始前の雑音声（音声調整・配信の演出など）が席を
# 先取りすると、後から来る実在の話者が満席で座れず、全発話が未確定（安全弁）
# か既存席への押し込み（安全弁導入前）になる。声紋が実在人物と同定したキーが
# 落ち続けており、かつ「発話がごく少なく長く黙っている席」があるときに限り、
# その席を回収して明け渡す。
# 校正（rehacq検証 2026-08-01 と既存16本の diag 実測）:
#   rehacq: 幽霊席=冒頭の音声調整の声（1発話7字・16秒以降無発話）。実在の
#   3人目（人物3）は 266秒から43回落ち続けた。この値なら3回目（約296秒、
#   幽霊は280秒無発話）で回収し、以後の40発話・2,500字超が本人に付く
#   千葉13本+AMI3本: 人数上限落ちが**0件**のため、回収は構造的に発火しない
#   （席の最小は17字のランがあるが、落ちる人物キー自体が生まれない）
# 誤発動の損害は有界: 回収された席の発話（上限20字）が未確定に戻るだけで、
# 誤帰属は生まない。回収先も声紋が同定した人物キーに限る。
SEAT_RECLAIM_MIN_DROPS = 3          # 人物キーがこの回数落ちたら回収を試みる
SEAT_RECLAIM_GHOST_MAX_CHARS = 20   # 回収対象の席の合計発話がこの字数以下
SEAT_RECLAIM_GHOST_IDLE_MS = 120_000  # かつ、この時間だれも発話を足していない
...
# 常駐ワーカーが状態を見に行く間隔（秒）。介入の判断・発話の取り込み・
# ファクトチェックの3つが同じ間隔で回る。応答の体感（人が話し終えてから
# AIが口を開くまで）はこの間隔ぶんだけ遅れうるので、詰めるほど機敏になるが
# 空回りも増える。0.25秒は「遅れが気にならない」と「CPUを無駄に使わない」の
# 折り合い。テストはここを小さくして待ち時間を減らす（本番の値は変えない）。
WORKER_TICK_SEC = 0.25
