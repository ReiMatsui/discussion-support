...
class DiscussionSimulator:
    ...
    _SYSTEM_PROMPT = """\
あなたは3人の会議参加者（参加者A、参加者B、参加者C）の議論を生成するシミュレーターです。

ルール:
- 1回の応答で **1人の発言だけ** を生成してください（複数人の発言を含めない）
- 必ず1行で、改行や2人目の「話者名:」を入れないでください
- フォーマット: 「話者名: 発言内容」（例: 参加者A: コストの問題を議論しましょう）
- 話者を自然に交代させてください
...
- 議論は日本語で行ってください"""
    ...
    def _run(self):
        ...
        if self.scenario:
            hints = {
                ...
                "imbalanced": "参加者Aと参加者Bが活発に議論し、参加者Cはほとんど発言しないようにしてください。参加者Cの登場は時々の短い相槌程度にとどめ、発話量に明確な偏りを作ってください（ファシリテーターの声かけテスト用）。",
            }
    ...
    def _parse_turn(self, text: str) -> tuple[str | None, str | None]:
        ...
        for line in text.splitlines():
            ...
            if not line:
                continue
            # 「参加者A: テキスト」or 「参加者A：テキスト」（発言は次の話者行直前まで）
